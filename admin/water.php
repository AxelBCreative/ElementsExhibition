...
?>	
	...
	<span class="span7">
		<h1>"Water"</h1>
		<h4>You are now editing: <br /> <a href="http://www.elementsexhibition.co.uk/elements/water">www.elementsexhibition.co.uk/elements/water</a></h4>
		<form action="water.php" name="submit" method="post" class="row">
			<label>title</label>
			...
			<?php if(isset($_POST['submit']))
{
	...
	$sql = "UPDATE `phuck_marketing`.`pages` SET  `title` =  '$title', `meta` =  '$meta', `heading` = '$heading', `shorturl` = '$shorturl', `image_one` = '$img1', `image_two` = '$img2', `image_three` = '$img3', `image_one_title` = '$img1cap', `image_two_title` = '$img2cap', `image_three_title` = '$img3cap' WHERE  `pages`.`name` = 'water';";

		mysql_query($sql); header("Location: http://localhost/ElementsExhibition/admin/water.php");
		}
		?>
		</form>	 
	</span>
